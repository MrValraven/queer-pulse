...
function SignInPane({ onSwitch }: { onSwitch: () => void }) {
  ...
  return (
    <div className={styles.pane}>
      ...
      <p className={styles.lede}>Studio is a tab on your QueerPulse account, not a new login. Sign in with the member account you already have.</p>

      <form onSubmit={handleSubmit}>
        <div className={styles.field}>
          <label htmlFor="si-email">Email</label>
          <input id="si-email" type="email" placeholder="you@example.com" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} />
        </div>
      ...
      <button className={styles.btnGoogle} onClick={() => showToast('Google sign-in coming soon', 'info')}>
...
function JoinPane({ onSwitch }: { onSwitch: () => void }) {
  ...
  return (
    <div className={styles.pane}>
      <h1>Join the <em>room.</em></h1>
      <p className={styles.lede}>Pick how much of the co-op you want. You can change tiers or cancel any month — no lock-in, no winback emails.</p>

      <form onSubmit={handleSubmit}>
        <div className={styles.field}>
          <label htmlFor="j-email">Email</label>
          <input id="j-email" type="email" placeholder="you@example.com" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} />
        </div>
        <div className={styles.field} style={{ marginBottom: 22 }}>
          <label htmlFor="j-pw">Choose a password</label>
          <input id="j-pw" type="password" placeholder="At least 8 characters" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} />
        </div>
        ...
        <div className={[styles.tier, tier === 'studio' ? styles.tierOn : ''].filter(Boolean).join(' ')} onClick={() => setTier('studio')}>
          ...
          <div className={styles.tierInfo}>
            <div className={styles.tierTop}>
              <h4>Studio <em>only</em></h4>
              <div className={styles.tierPrice}>€<em>7</em><b>/mo</b></div>
            </div>
            <p>Everything in Studio — the council's weekly set, live rooms, the full catalogue, lossless audio, direct artist subscriptions.</p>
        ...
        <div className={[styles.tier, tier === 'coop' ? styles.tierOn : ''].filter(Boolean).join(' ')} style={{ position: 'relative' }} onClick={() => setTier('coop')}>
          ...
          <div className={styles.tierInfo}>
            <div className={styles.tierTop}>
              <h4>The whole <em>co-op</em></h4>
              <div className={styles.tierPrice}>€<em>11</em><b>/mo</b></div>
            </div>
            <p>Studio <em>plus</em> Cinema, the Magazine, Gatherings, reading groups, and a vote at the annual assembly. One membership, the entire QueerPulse.</p>
            <div className={styles.tierIncl}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.4}><path d="M20 6L9 17l-5-5" /></svg>
        ...
        <Button type="submit" className={styles.btnFull} style={{ marginTop: 8 }} disabled={!canSubmit}>
          Continue to payment →
        </Button>
      </form>
    ...
    <div className={styles.root}>
      <aside className={styles.aside}>
        <div className={styles.asideBrand}>
          ...
          <span className={styles.wordmark}>Queer<em>Pulse</em></span>
        ...
        <div className={styles.asideBody}>
          ...
          <h2>A streaming co-op that <em>pays</em> the people who made the song.</h2>
          <p className={styles.asideSub}>Eighty cents of every euro reaches the artist. Tips pass through at <em>100%</em>. The ledger is public, updated every Monday at noon.</p>
          <div className={styles.nowPlaying}>
            <div className={styles.cover} />
            <div className={styles.nowInfo}>
              <div className={styles.nowTitle}>Carta para a <em>santa</em></div>
              <div className={styles.nowArtist}>Mariana Sol · Cidade dos santos</div>
            </div>
            <div className={styles.bars}>
              <span /><span /><span /><span />
            </div>
          </div>
          <div className={styles.asideFoot}>Paid to artists this month: <em>€11,940</em> · and counting.</div>
        </div>
      </aside>
      ...
      <main className={styles.authMain}>
        <div className={styles.authCard}>
          ...
          <div className={styles.segTabs}>
            <button className={tab === 'in' ? styles.segOn : ''} onClick={() => setTab('in')}>Sign in</button>
            <button className={tab === 'join' ? styles.segOn : ''} onClick={() => setTab('join')}>Join</button>
          </div>
