    ...
    <StudioShell>
      <div className={s.wrap}>
        <div className={s.hero}>
          <div className={`${s.eb} ${s.ebAccent}`}>New submission · DJ set or mix</div>
          <h1>
            Submit a <em>set</em>.
          </h1>
          <div className={s.dek}>
        ...
        <div className={s.djGrid}>
          <div>
            ...
            <div className={s.secLbl}>Paste your tracklist · timecode — artist — title</div>
                  ...
                  {TRACKS.map((t, i) => (
                    <div key={i} className={s.mtRow}>
                      ...
                      <div>
                        ...
                        <p>{t.m ? <>{t.who} · <em>€0.05/play to {t.who.split(' ')[0]}</em></> : 'no source found · payout held'}</p>
                      </div>
                      {t.m ? (
                        ...
                        <span className={s.st}>
                          <span className={s.resolve} onClick={() => showToast('Search opened to identify this track', 'info')}>
                            Identify
                          </span>
                        </span>
                      )}
          ...
          <div className={s.djSide}>
            ...
            <div className={s.djCard}>
              ...
              <div className={s.djRow}>
                ...
                <span className="v" style={{ fontFamily: 'var(--serif)', color: 'var(--cream)' }}>{TRACKS.length}</span>
              </div>
              <div className={s.djRow}>
                <span>Matched &amp; paying</span>
                <span className={`${s.v} jade`} style={{ fontFamily: 'var(--serif)', color: 'var(--jade-light)' }}>{matched} tracks</span>
              </div>
              <div className={s.djRow}>
                <span>On hold (unmatched)</span>
                <span className={`${s.v} warn`} style={{ fontFamily: 'var(--serif)', color: 'var(--accent)' }}>{held} tracks</span>
              </div>
              <div className={s.djRow}>
                <span>Set payout pool</span>
                <span className="v" style={{ fontFamily: 'var(--serif)', color: 'var(--cream)' }}>
                  €<em style={{ fontStyle: 'normal', color: 'var(--accent)' }}>11.40</em>
                </span>
              </div>
            ...
            <button className={`${s.bt} ${s.btJade} ${s.btLg}`} onClick={() => showToast('Set submitted — live now, held shares pending clearance', 'success')}>
              Submit set →
            </button>
          </div>
        </div>
