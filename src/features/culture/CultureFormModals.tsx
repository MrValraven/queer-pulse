...
import { ModalShell, SuccessPanel, Sending, ChipSelect, useChipSet, useSubmitFlow } from './CultureModalKit'
import {
  PICK_KINDS,
  PROJECT_LOOKING_FOR,
  SHOWCASE_MEDIUMS,
...
function SubmitBtn({ sending, label, onClick }: { sending: boolean; label: string; onClick: () => void }) {
  return (
    ...
    <ModalShell onClose={onClose} success={done}>
      {done ? (
        ...
        <>
          ...
          <p className={styles.sub}>Nominate something for the community to vote on this month.</p>
          ...
          <div className={styles.field}>
            ...
            <input id="pk-title" type="text" placeholder="e.g. Detransition, Baby" />
          </div>
          <div className={styles.field}>
          ...
          <div className={styles.field}>
            ...
            <textarea id="pk-why" placeholder="A sentence on why the club should pick it…" />
          </div>
          <div className={styles.foot}>
            <Button variant="ghost" onClick={onClose}>
              Cancel
            </Button>
            <SubmitBtn sending={sending} label="Nominate pick" onClick={() => submit()} />
          </div>
        </>
      )}
    </ModalShell>
    ...
    <ModalShell onClose={onClose} success={done}>
      {done ? (
        ...
        <>
          ...
          <p className={styles.sub}>Tell the community what you're making and who you need.</p>
          <div className={styles.field}>
            <label htmlFor="pp-title">Project title</label>
            <input id="pp-title" type="text" placeholder="What are you making?" />
          </div>
          <div className={styles.field}>
            <label htmlFor="pp-desc">Description</label>
            <textarea id="pp-desc" placeholder="What it is, where you're at, and what collaboration looks like…" />
          </div>
          <div className={styles.field}>
            <label>Looking for</label>
            <ChipSelect options={PROJECT_LOOKING_FOR} selected={selected} onToggle={toggle} />
          </div>
          <div className={styles.foot}>
            <Button variant="ghost" onClick={onClose}>
              Cancel
            </Button>
            <SubmitBtn sending={sending} label="Post project" onClick={() => submit()} />
          </div>
        </>
      )}
    </ModalShell>
      ...
      {done ? (
        <SuccessPanel
          ...
          steps={[
            ...
            <>You'll hear back by <strong>{replyByDate(4)}</strong>, selected or not.</>,
          ]}
          onClose={onClose}
        >
        ...
        <>
          ...
          <p className={styles.sub}>Up to three pieces. Reviewed monthly by member curators.</p>
          <div className={styles.field}>
            <label htmlFor="sw-title">Title of the work</label>
            <input id="sw-title" type="text" placeholder="e.g. Corpo Estranho, 2024" />
          </div>
          <div className={styles.field}>
          ...
          <div className={styles.field}>
            ...
            <input id="sw-link" type="url" placeholder="Portfolio, image or video URL" />
          </div>
          <div className={styles.field}>
            <label htmlFor="sw-about">About the piece</label>
            <textarea id="sw-about" placeholder="A short statement the curators can read…" />
          </div>
          <div className={styles.foot}>
            <Button variant="ghost" onClick={onClose}>
              Cancel
            </Button>
            <SubmitBtn sending={sending} label="Submit work" onClick={() => submit()} />
          </div>
        </>
      )}
    </ModalShell>
      ...
      {done ? (
        <SuccessPanel
          ...
          steps={[
            ...
            <>If it's a fit, we'll book you a slot and email you by <strong>{replyByDate(3)}</strong>.</>,
          ]}
          onClose={onClose}
        >
        ...
        <>
          ...
          <p className={styles.sub}>Pitch a set for a guest DJ slot. No ads, no algorithm.</p>
          <div className={styles.field}>
            <label htmlFor="pl-name">Playlist name</label>
            <input id="pl-name" type="text" placeholder="e.g. A noite que ficou em Lisboa" />
          </div>
          <div className={styles.field}>
            <label htmlFor="pl-link">Link to the set</label>
            <input id="pl-link" type="url" placeholder="Spotify, SoundCloud or a tracklist URL" />
          </div>
          <div className={styles.field}>
            <label>Vibe</label>
            <ChipSelect options={PLAYLIST_VIBES} selected={selected} onToggle={toggle} />
          </div>
          <div className={styles.field}>
            <label htmlFor="pl-note">Curator's note</label>
            <textarea id="pl-note" placeholder="When is this for? What's it about?…" />
          </div>
          <div className={styles.foot}>
            <Button variant="ghost" onClick={onClose}>
              Cancel
            </Button>
            <SubmitBtn sending={sending} label="Submit playlist" onClick={() => submit()} />
          </div>
        </>
      )}
    </ModalShell>
