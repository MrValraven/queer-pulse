    ...
    <div className={styles.col}>
      <div className={styles.card}>
        ...
        <div className={styles.cardBody}>
          ...
          <input className={styles.nameSearch} type="text" placeholder="Search guest list…" value={scanQuery} onChange={(e) => setScanQuery(e.target.value)} />
          {scanMatches !== null && (
...
export function GuestListCard({
  ...
  const visible = useMemo(
    () =>
      guests.filter(
        (g) => (filter === 'all' || g.status === filter) && (!query || g.name.toLowerCase().includes(query.toLowerCase())),
      ),
    [guests, filter, query],
  )
      ...
      <div className={styles.cardBody}>
        <div className={styles.filterBar}>
          {(
            ...
            <button key={id} type="button" className={[styles.afBtn, filter === id && styles.afBtnActive].filter(Boolean).join(' ')} onClick={() => setFilter(id)}>
              {label}
            </button>
          ))}
        </div>
          ...
          {visible.map((g) => (
            <div className={styles.attRow} key={g.name}>
              ...
              <div className={styles.attInfo}>
                ...
                {g.status === 'pending' && (
                  <button type="button" className={styles.manualBtn} onClick={() => onCheckIn(g.name)}>
                    Check in manually
                  </button>
                )}
              </div>
          ...
          <div>
            {WAITLIST.map((w) => (
              <div className={styles.attRow} key={w.name}>
                ...
                <button type="button" className={styles.promoteBtn} onClick={() => showToast(`${w.name.split(' ')[0]} promoted to guest list`, 'success')}>
                  Promote
                </button>
              </div>
            ))}
