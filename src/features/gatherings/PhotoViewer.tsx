  ...
  return (
    ...
    >
      ...
      <div className={styles.stage}>
        <button type="button" className={styles.nav} onClick={() => go(-1)} aria-label="Previous photo">
          <FiChevronLeft />
        </button>

        <div className={[styles.frame, styles[pic.tint]].join(' ')}>
          {pic.tag && (
            <span className={[styles.tag, pic.tagAccent && styles.tagAccent].filter(Boolean).join(' ')}>
              {pic.tag}
            </span>
          )}
          ...
          <div className={styles.progress}>
            {pics.map((_, i) => (
              <span key={i} className={[styles.dot, i === index && styles.dotActive].filter(Boolean).join(' ')} />
            ))}
          </div>
        </div>

        <button type="button" className={styles.nav} onClick={() => go(1)} aria-label="Next photo">
          <FiChevronRight />
        </button>
      </div>
    </div>
