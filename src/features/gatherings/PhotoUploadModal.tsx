...
export function PhotoUploadModal({ onClose, onSubmit }: PhotoUploadModalProps) {
  ...
  return (
    <div className={styles.overlay} onClick={onClose}>
      <div
        className={done ? `${styles.dialog} ${styles.dialogConfirm}` : styles.dialog}
        role="dialog"
        aria-modal="true"
      ...
      >
        {done ? (
          ...
          >
            ...
            <div className={styles.fileRow}>
              {SAMPLE_FILES.map((f, i) => (
                <button
                  key={f.label}
                  type="button"
                  className={[styles.fileBtn, picked === i && styles.fileBtnOn].filter(Boolean).join(' ')}
                  onClick={() => setPicked(i)}
                >
            ...
            <label className={styles.fieldLabel} htmlFor="photo-caption">Caption</label>
            <input
              id="photo-caption"
            ...
            <div className={styles.dialogActions}>
              ...
              <Button variant="primary" type="submit">Add photo</Button>
            </div>
          </form>
        )}
      </div>
