...
export function TypeStep({ form }: { form: GatheringForm }) {
  return (
    <div>
      ...
      <p className={styles.stepSub}>Choose the format. This determines some of the fields that follow.</p>
    ...
    <div>
      ...
      <div className={styles.row2}>
        <div>
          ...
          <input className={styles.input} type="date" value={form.date} onChange={(e) => form.setDate(e.target.value)} />
        </div>
        <div>
          <label className={styles.label}>Time</label>
          <input className={styles.input} type="time" value={form.time} onChange={(e) => form.setTime(e.target.value)} />
        </div>
      </div>
      <div className={styles.row2}>
        <div>
          <label className={styles.label}>End time (optional)</label>
          <input className={styles.input} type="time" value={form.endTime} onChange={(e) => form.setEndTime(e.target.value)} />
        </div>
        <div>
          <label className={styles.label}>Neighbourhood</label>
          <select className={styles.select} value={form.hood} onChange={(e) => form.setHood(e.target.value)}>
      ...
      <input className={styles.input} type="text" placeholder="e.g. Casa da Mariquinhas, My studio, Jardim do Torel" value={form.venue} onChange={(e) => form.setVenue(e.target.value)} />
      <label className={styles.label}>Full address (shared only with confirmed attendees)</label>
      <input className={styles.input} type="text" placeholder="Street address" value={form.address} onChange={(e) => form.setAddress(e.target.value)} />
      <label className={styles.label}>Getting there (optional)</label>
      <input className={styles.input} type="text" placeholder="e.g. Ring the bell on the left, 5 min walk from Intendente metro" value={form.directions} onChange={(e) => form.setDirections(e.target.value)} />
    </div>
  )
}
    ...
    <div>
      ...
      <div className={styles.row2}>
        <div>
          ...
          <input className={styles.input} type="number" min={2} max={200} placeholder="Max attendees" value={form.cap} onChange={(e) => form.setCap(e.target.value)} />
        </div>
        <div>
          <label className={styles.label}>Language</label>
          <select className={styles.select} value={form.lang} onChange={(e) => form.setLang(e.target.value)}>
            {LANGS.map((l) => (
      ...
      <p className={styles.hint}>Only tick what you can genuinely confirm. Attendees will rely on this information.</p>
      ...
      <input className={styles.input} type="text" placeholder="Anything else attendees should know — steps, parking, sound level…" value={form.accessNotes} onChange={(e) => form.setAccessNotes(e.target.value)} />
    </div>
  )
}
...
export function PricingStep({ form }: { form: GatheringForm }) {
  return (
    <div>
      ...
      {!form.free && (
        <div>
          <p className={styles.hint}>
            Set three tiers. The sliding scale is not optional — if your event is paid, all three
            tiers must be offered. Members choose their tier privately.
          </p>
          ...
          <div className={styles.tierRow}>
            ...
            <div className={styles.moneyCell}>
              <span className={styles.moneyPrefix}>€</span>
              <input className={styles.tierInput} type="number" min={0} value={form.stdPrice} onChange={(e) => form.setStdPrice(e.target.value)} aria-label="Standard price in euros" />
            </div>
            <div className={styles.spotsCell}>
          ...
          <div className={styles.tierRow}>
            ...
            <div className={styles.moneyCell}>
              <span className={styles.moneyPrefix}>€</span>
              <input className={styles.tierInput} type="number" min={0} value={form.supPrice} onChange={(e) => form.setSupPrice(e.target.value)} aria-label="Supporter price in euros" />
            </div>
            <div className={styles.spotsCell}>
      ...
      <input className={styles.input} type="text" placeholder="e.g. Shared dinner and wine, materials provided, just your time" value={form.included} onChange={(e) => form.setIncluded(e.target.value)} />
      <label className={styles.label}>Anything to bring / prepare?</label>
      <input className={styles.input} type="text" placeholder="Optional — e.g. Bring something to share, wear comfortable shoes" value={form.bring} onChange={(e) => form.setBring(e.target.value)} />
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <p className={styles.checkIntro}>
        Tick each box to confirm. The <strong>Publish gathering</strong> button stays disabled until
