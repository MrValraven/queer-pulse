...
export function ModToolsTab({ living }: { living: LivingCommunity }) {
  ...
  const resolveRequest = (id: string, name: string, approved: boolean) => {
    ...
    showToast(
      approved ? `${name} approved — welcome them in.` : `${name}'s request wasn't approved this time.`,
      approved ? 'success' : 'info',
    )
  }
  const resolveReport = (id: string, removedPost: boolean) => {
    setReports((prev) => prev.filter((r) => r.id !== id))
    showToast(removedPost ? 'Post removed. The author has been reached.' : 'Report dismissed.', removedPost ? 'success' : 'info')
  }
  ...
  const manageable = living.roster.filter((m) => !removed.includes(memberKey(m.slug, m.name)))

  return (
    <div>
      {/* Join requests */}
      <div className={detail.secLbl}>
        People asking to join {requests.length > 0 && <span className={detail.tabCount}>{requests.length}</span>}
      </div>
      {requests.length === 0 ? (
        <EmptyState compact title="No requests waiting" description="You're all caught up — new requests will appear here." />
      ) : (
        requests.map((r) => (
          <div className={styles.modRow} key={r.id}>
            <Avatar initials={r.person.initials} tint={r.person.tint} src={photoOf(r.person)} size={42} alt={r.person.name} />
            ...
            <div className={styles.modActions}>
              <Button variant="jade" onClick={() => resolveRequest(r.id, r.person.name, true)}>
      ...
      <div className={detail.secLbl} style={{ marginTop: 32 }}>
        Reported posts {reports.length > 0 && <span className={detail.tabCount}>{reports.length}</span>}
      </div>
      {reports.length === 0 ? (
        <EmptyState compact title="All clear" description="Nothing has been flagged — the community looks after each other." />
      ) : (
        reports.map((rep) => (
          <div className={styles.reportCard} key={rep.id}>
            ...
            <div className={styles.modActions} style={{ marginTop: 12 }}>
              <Button variant="primary" onClick={() => resolveReport(rep.id, true)}>
                Remove post
              </Button>
              <span
                role="button"
      ...
      {manageable.map((m) => {
        ...
        return (
          <div className={styles.modRow} key={key}>
            <Avatar initials={m.initials} tint={m.tint} src={photoOf(m)} size={38} alt={m.name} />
            <div className={styles.modMain}>
              <div className={styles.modName}>
                {m.name} <RoleBadge role={promoted.includes(key) ? 'mod' : m.role} />
              </div>
