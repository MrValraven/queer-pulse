...
import { Button, FadeIn, EmptyState, SkeletonAvatar, SkeletonLine } from '../../shared/components/ui'
...
export function CommunitiesHomePage() {
  ...
  const pulse = interleave(
    mine
      .filter((m) => m.living)
      .map((m) =>
        [...m.living!.pinned, ...m.living!.pulse]
          .slice(0, 4)
          .map((post) => ({ post, communityName: m.community?.name ?? m.slug, communitySlug: m.slug })),
      ),
  ).slice(0, 8)
  ...
  const upcoming: UpcomingItem[] = mine
    .filter((m) => m.living)
    .flatMap((m) =>
      m.living!.events.filter((e) => !e.past).map((event) => ({ event, name: m.community?.name ?? m.slug, slug: m.slug })),
    )
    .slice(0, 4)
  ...
  const livingMine = mine.map((m) => m.living).filter(Boolean) as NonNullable<(typeof mine)[number]['living']>[]
  const digest = {
    posts: livingMine.reduce((s, c) => s + c.stats.postsThisWeek, 0),
    active: livingMine.reduce((s, c) => s + c.stats.activeThisWeek, 0),
    events: upcoming.length,
    joined: livingMine.reduce((s, c) => s + c.moments.filter((mo) => mo.kind === 'joined').length, 0),
  }
        ...
        <div className="wrap">
          <div className={styles.head}>
            <div>
              ...
              <p className={styles.sub}>Here's what's been happening across your {myCommunities.length} communities.</p>
            </div>
            ...
            <>
              <div className={styles.digest}>
                <div className={styles.digestSide}>
                  ...
                  <p className={styles.digestNote}>No pings — just your communities' week in one glance.</p>
                </div>
              ...
              <div>
                {todos.length > 0 && (
                  <div className={styles.todos}>
                    ...
                    {todos.map((t) => (
                      <Link key={t.slug} to={`/community/${t.slug}`} className={styles.todoRow}>
                        <span className={styles.todoName}>{t.name}</span>
                        <span className={styles.todoCounts}>
                          {t.requests > 0 && (
                            <span className={styles.todoChip}>
                              <FiInbox aria-hidden /> {t.requests} request{t.requests > 1 ? 's' : ''}
                            </span>
                          )}
                          {t.reports > 0 && (
                            <span className={[styles.todoChip, styles.todoChipFlag].join(' ')}>
                              <FiFlag aria-hidden /> {t.reports} report{t.reports > 1 ? 's' : ''}
                            </span>
                          )}
                        </span>
                      </Link>
                ...
                {loading ? (
                  ...
                  <EmptyState compact title="Quiet for now" description="When your communities post, it shows up here." />
                ) : (
                  pulse.map((item, i) => (
              ...
              <CommunitiesHomeSidebar communities={myCommunities} upcoming={upcoming} suggestions={suggestions} />
              </div>
            </>
          )}
        </div>
