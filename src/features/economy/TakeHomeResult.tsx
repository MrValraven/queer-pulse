...
import { SIMPLIFIED_COEFFICIENTS, SS_RATE_ENI, SS_RATE_FREELANCER, TAX_DISCLAIMER } from './tax.constants'
...
export function TakeHomeResult({ gross, activity, year, startupYear, status }: TakeHomeResultProps) {
  ...
  const keptPct = grossNum > 0 ? Math.max(0, Math.min(100, (result.net / grossNum) * 100)) : 0
  ...
  return (
    <div className={styles.result}>
      <div className={styles.netPanel}>
        ...
        <p className={styles.netLabel}>You take home, after IRS &amp; Segurança Social</p>
        <p className={styles.netValue}>{euro(result.net)}</p>
        <p className={styles.netMonthly}>
          ≈ <strong>{euro(result.net / 12)}</strong> a month
        </p>

        <div className={styles.bar} role="img" aria-label={`You keep ${Math.round(keptPct)}% of your gross income`}>
          <div className={styles.barKept} style={{ width: `${keptPct}%` }} />
        </div>
        <p className={styles.barCaption}>
          You keep <em>{Math.round(keptPct)}%</em> of every euro you bill. Effective IRS + SS rate:{' '}
          {ratePct}%.
        </p>
      </div>
