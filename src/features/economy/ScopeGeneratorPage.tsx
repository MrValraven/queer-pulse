...
function toPlainText(scope: ScopeState, issuerName: string): string {
  ...
  scope.deliverables.filter((d) => d.trim()).forEach((d) => lines.push(`  + ${d}`))
  lines.push('')

  lines.push('Not included')
  scope.outOfScope.filter((d) => d.trim()).forEach((d) => lines.push(`  - ${d}`))
  lines.push('')
...
export function ScopeGeneratorPage() {
  ...
  const update = (patch: Partial<ScopeState>) => setScope((prev) => ({ ...prev, ...patch }))
