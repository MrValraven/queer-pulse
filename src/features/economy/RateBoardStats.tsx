...
function groupByRole(entries: RateEntry[]): RoleStat[] {
  ...
  return [...byRole.entries()]
    .map(([role, rates]) => ({ role, count: rates.length, med: median(rates), lo: min(rates), hi: max(rates) }))
    ...
    return (
      <div className={styles.statsEmpty}>
        ...
        <p className={styles.emptyBody}>
          Be the first to add a rate, or import a JSON file someone shared with you. The
          distribution shows up here as soon as there's data.
        </p>
      </div>
    )
  }
    ...
    <div className={styles.stats}>
      ...
      {showCompare && (
        <div className={styles.percentile}>
          <span className={styles.pctLabel}>Your rate of {euro(compareRate)} sits at the</span>
