...
export function MentorApplyModal({
  ...
  const toggle = (area: string) => setFocus((cur) => (cur === area ? null : area))

  return (
    <ModalShell onClose={onClose} success={done}>
      {done ? (
        <SuccessPanel title="Request" em="sent." onClose={onClose} closeLabel="Done">
          Your request to work with <strong>{mentorName}</strong> on <strong>{focus}</strong> is on
          its way. She reviews applications for the '26 cohort personally and usually replies within a
          week. You'll get a notification here.
        </SuccessPanel>
      ) : (
        <>
          ...
          <p className={styles.sub}>
            Tell her where you'd like to focus and a little about your work. There's no upfront cost —
            mentees get a free Sustainer membership for the year.
          </p>
            ...
            <div className={styles.levers}>
              {APPLY_FOCUS_AREAS.map((area) => (
                <button
                  key={area}
                  type="button"
                  className={[styles.lever, focus === area && styles.leverOn].filter(Boolean).join(' ')}
                  onClick={() => toggle(area)}
          ...
          <div className={styles.field}>
            <label htmlFor="ma-msg">A note to {mentorName.split(' ')[0]} *</label>
            <textarea
              id="ma-msg"
              value={message}
