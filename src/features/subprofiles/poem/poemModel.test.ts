...
describe("normalizePoemBlocks", () => {
  ...
  it("keeps a break block as-is", () => {
    expect(normalizePoemBlocks([{ kind: "break", id: "b" }])).toEqual([
      { kind: "break", id: "b" },
    ]);
  });
  it("returns [] for junk", () => {
    ...
    const text = poemToPlainText([
      {
        kind: "stanza",
        id: "1",
        lines: [[{ text: "a", marks: [] }], [{ text: "b", marks: [] }]],
      },
      { kind: "break", id: "2" },
