...
import {
  detectMergedStanzaLine,
  splitMergedLineIntoLines,
} from "./poemResplit";
...
import { usePoemFocusAcrossMove } from "./usePoemFocusAcrossMove";
 ...
 *  which comfortably fits two reading-width columns above it). */
...
export function PoemBodyEditor({
  ...
  // Handles for the mounted stanza/note fields, keyed by block id. They let an
  ...
  const focusAcrossMove = usePoemFocusAcrossMove();
  // Bumped on every move, so the blocks glide on moves only (`ReorderRow`).
  ...
  function reorderBlocks(fromIndex: number, toIndex: number) {
    const isOutside = (index: number) => index < 0 || index >= blocks.length;
    if (isOutside(fromIndex) || isOutside(toIndex)) return;
    const next = [...blocks];
    [next[fromIndex], next[toIndex]] = [next[toIndex]!, next[fromIndex]!];
    // A drag or a move button re-inserts the block's node, which drops the
    // focus (and a stanza's caret) inside it; the next commit puts it back.
    focusAcrossMove.remember();
  ...
  // Kept as a single JSX value so it is written once and rendered from
  // exactly one place per breakpoint: desktop always mounts it
  // (caret-preserving), mobile mounts it only on the "edit" tab.
  const editorPane = (
