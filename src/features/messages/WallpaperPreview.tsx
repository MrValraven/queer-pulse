 ...
 *
 * It carries the same `data-wallpaper-pattern` attribute the conversation
 * panel does and reuses the same tokens from chat-wallpaper.css, so what shows
 * here is what the log will look like. The bubble styling is deliberately
 * local rather than imported from MessagesPage.module.css: this is a
 * miniature (the real bubbles carry avatars, timestamps, receipts and
 * reaction slots), and pulling in the real classes would drag all of that
 * with it.
 */
export function WallpaperPreview({ choice }: { choice: WallpaperChoice }) {
  ...
  return (
    <div
      ...
      data-wallpaper-pattern={choice.pattern}
