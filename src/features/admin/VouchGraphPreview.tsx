...
export function VouchGraphPreview({ focusId }: { focusId: string }) {
  ...
  return (
    ...
    >
      <defs>
        <clipPath id="pvClipC"><circle r={CENTER_R} /></clipPath>
        <clipPath id="pvClipN"><circle r={NODE_R} /></clipPath>
      </defs>
      ...
      {placed.map((p) => {
        ...
        const x1 = CX + ux * CENTER_R, y1 = CY + uy * CENTER_R
        const x2 = p.x - ux * NODE_R, y2 = p.y - uy * NODE_R
        ...
        return <path key={p.id} className={cls} fill="none" d={`M${x1} ${y1}Q${cx} ${cy} ${x2} ${y2}`} />
      })}

      {placed.map((p) => (
        <PreviewNode key={p.id} id={p.id} x={p.x} y={p.y} r={NODE_R} clip="pvClipN" />
      ))}
      <PreviewNode id={focusId} x={CX} y={CY} r={CENTER_R} clip="pvClipC" focus />
    </svg>
  )
}
...
function PreviewNode({ id, x, y, r, clip, focus = false }: { id: string; x: number; y: number; r: number; clip: string; focus?: boolean }) {
  ...
  return (
    <g transform={`translate(${x} ${y})`}>
      ...
      {photo ? (
        <>
          <image href={photo} x={-r} y={-r} width={r * 2} height={r * 2} clipPath={`url(#${clip})`} preserveAspectRatio="xMidYMid slice" />
          <circle r={r} fill="none" stroke={ink.stroke} strokeWidth={sw} />
        </>
      ) : (
        <>
          <circle r={r} fill={ink.fill} stroke={ink.stroke} strokeWidth={sw} />
          <text className={styles.previewInit} dy={r / 3} fontSize={focus ? r * 0.6 : r * 0.7} fill={p.anon ? 'var(--ink-40)' : ink.stroke}>
            {p.initials}
          </text>
        </>
      )}
      {verified && (
        <>
          <circle cx={r * 0.72} cy={r * 0.72} r={4.5} fill="var(--jade)" stroke="var(--paper)" strokeWidth={2} />
          <path d={`M ${r * 0.72 - 2} ${r * 0.72} l 1.4 1.4 l 2.6 -3`} stroke="var(--paper)" strokeWidth={1.3} fill="none" />
        </>
      )}
    </g>
  )
