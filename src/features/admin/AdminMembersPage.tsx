...
export function AdminMembersPage() {
  ...
  return (
    <AdminShell title={<>Members · <em>the people</em></>}>
      <FadeIn>
        <AdminPageHeader
          eyebrow="Member directory"
          title={
            <>
              8,412 people,<br />each one <em>vouched for</em>.
            </>
          }
      ...
      <FadeIn delay={80}>
        <div className={styles.toolbar}>
          ...
          {tab === 'all' && (
            <div className={styles.filters} role="group" aria-label="Filter members">
              {FILTERS.map((f) => (
                <button
                  key={f.id}
                  type="button"
      ...
      <FadeIn delay={140}>
        {tab === 'all' && <AdminMemberRows members={visibleMembers} onSelect={setSelected} />}
        {tab === 'pending' && <AdminVerifyQueue />}
        {tab === 'flagged' && <AdminFlaggedRows members={FLAGGED} />}
      </FadeIn>

      {selected && <AdminMemberDrawer member={selected} onClose={() => setSelected(null)} />}
    </AdminShell>
  )
}
