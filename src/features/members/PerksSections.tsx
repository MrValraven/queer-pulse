...
function CheckMini() {
  return (
    <svg width="11" height="11" viewBox="0 0 11 11" fill="none" aria-hidden>
      <path d="M2 5.5l2.5 2.5 4.5-5" stroke="var(--jade)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
function AutoCheck() {
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden>
      <path d="M2 6l3 3 6-5" stroke="var(--jade)" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
function LockMini() {
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden>
      <rect x="2" y="5.5" width="8" height="6" rx="1" stroke="currentColor" strokeWidth="1.3" />
...
function PerkCard({ perk }: { perk: Perk }) {
  ...
  return (
    <article className={`${styles.perkCard} ${dimmed ? styles.perkCardDim : ''}`}>
      <div className={`${styles.pcBar} ${barClass[perk.state]}`} />
      <div className={styles.pcBody}>
        <div className={`${styles.pcCat} ${catClass[perk.state]}`}>{perk.cat}</div>
        <h3 className={`${styles.pcTitle} ${dimmed ? styles.pcTitleDim : ''}`}>{perk.title}</h3>
        <p className={`${styles.pcDesc} ${dimmed ? styles.pcDescDim : ''}`}>{perk.desc}</p>
    ...
    <aside className={styles.sidebar}>
      <div className={styles.sbCard}>
        ...
        <div className={styles.miniXpBar}>
          <div className={styles.miniXpFill} style={{ width: `${levelInfo.percent}%` }} />
        </div>
