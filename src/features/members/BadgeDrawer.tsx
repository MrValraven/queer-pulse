...
import { useBadgeStoryNotes, useBadgeVisibilityPrefs } from "./useBadgePreferences";
import styles from "./BadgesPage.module.css";

const VERIFY_COPY: Record<BadgeVerification, { titleKey: string; bodyKey: string }> = {
  auto: { titleKey: "members:badges.drawer.verifyAutoTitle", bodyKey: "members:badges.drawer.verifyAutoBody" },
  host: { titleKey: "members:badges.drawer.verifyHostTitle", bodyKey: "members:badges.drawer.verifyHostBody" },
  review: { titleKey: "members:badges.drawer.verifyReviewTitle", bodyKey: "members:badges.drawer.verifyReviewBody" },
  peer: { titleKey: "members:badges.drawer.verifyPeerTitle", bodyKey: "members:badges.drawer.verifyPeerBody" },
};
...
export function BadgeDrawer({ entries, index, onNavigate, onClose }: BadgeDrawerProps) {
  ...
  return (
    <SideSheet title={badge.name} onClose={onClose}>
      <div className={styles.drNav}>
        <button type="button" className={styles.drIb} onClick={() => onNavigate((index - 1 + entries.length) % entries.length)} aria-label={t("members:badges.drawer.previous")}>
          <FiChevronLeft aria-hidden />
        </button>
        <button type="button" className={styles.drIb} onClick={() => onNavigate((index + 1) % entries.length)} aria-label={t("members:badges.drawer.next")}>
          <FiChevronRight aria-hidden />
        </button>
        <span className={styles.drCount}>{t("members:badges.drawer.count", { index: index + 1, total: entries.length })}</span>
      </div>

      <div className={styles.drHero}>
        <BadgeMedallion badge={badge} earned={earned} progressPercent={percent} size="big" />
        <div className={styles.drCat}>{badge.seasonal ? t("members:badges.drawer.seasonalTag") : badge.category}</div>
        <h3>{badge.name}</h3>
        <p className={styles.drStatus}>
          {earned
            ? badge.when || t("members:badges.drawer.earnedThisWeek")
            : badge.progress
              ? t("members:badges.drawer.progressCount", { units: badge.progress.units, target: badge.progress.target })
      ...
      <div className={styles.drSec}>
        ...
        <div className={`${styles.critRow} ${earned ? styles.critRowOn : ""}`}>
          <span className={styles.critIcon}>{earned && <FiCheck aria-hidden />}</span>
          <span>{badge.criteria ?? badge.when}</span>
        </div>
        <div className={styles.drStrip}>
          <div>
            <div className={styles.drStripKey}>{t("members:badges.drawer.rarity")}</div>
            <div className={`${styles.drStripVal} ${badge.rarity === "legendary" ? styles.drStripValRare : ""}`}>
              {t(RARITY_LABEL_KEY[badge.rarity])}
            </div>
          </div>
          {badge.xpReward !== undefined && (
            <div>
              <div className={styles.drStripKey}>{t("members:badges.drawer.worth")}</div>
              <div className={styles.drStripVal}>{badge.xpReward}</div>
            </div>
          )}
          {badge.verifiedBy && (
            <div>
              <div className={styles.drStripKey}>{t("members:badges.drawer.checkedBy")}</div>
      ...
      {earned && (
        <div className={styles.drSec}>
          ...
          />
          <p className={styles.noteHelp}>{t("members:badges.drawer.noteHelp")}</p>
        </div>
      )}
      ...
      {earned && (
        <div className={styles.drFoot}>
          <div className={styles.drPriv}>
            <span>
              {visible ? t("members:badges.drawer.visibleOnProfile") : t("members:badges.drawer.privateToYou")}
              <span className={styles.drPrivNote}>{t("members:badges.drawer.visibilityNote")}</span>
            </span>
            <button
              type="button"
