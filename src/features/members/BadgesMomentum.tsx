...
import type { RecognitionLevel, XpBreakdownItem } from "./api/recognition.adapters";
...
export function BadgesMomentum({
  ...
  const nearest = closestToEarning(lockedBadges, mutedCategories).slice(0, MOMENTUM_COUNT);
  ...
  const entries: BadgeDrawerEntry[] = nearest.map((badge) => ({ badge, earned: false }));

  return (
    <section className={styles.sec}>
      <div className={styles.hd}>
        <div>
          <span className={styles.hdEyebrow}>{t("members:badges.momentum.eyebrow")}</span>
          <h2 className={`${styles.hdTitle} ${styles.hdLvl1}`}>
            <Translation i18nKey="members:badges.momentum.heading" components={{ em: <em /> }} />
          </h2>
        </div>
        {nearest[0] && (
          <Button variant="ghost" size="sm" onClick={() => setPreviewBadge(nearest[0] ?? null)}>
      ...
      {nearest.length > 0 ? (
        <div className={styles.nxGrid}>
          {nearest.map((badge, index) => {
            ...
            const tint = badge.tint === "jade" ? styles.tJade : badge.tint === "accent" ? styles.tCoral : styles.tPlum;
            return (
              <article key={badge.key} className={`${styles.nx} ${tint}`}>
                <span className={styles.nxXp}>
                  {t("members:badges.momentum.xpReward", { xp: badge.xpReward ?? 0 })}
                </span>
                <button
                  type="button"
                ...
                >
                  <span className={styles.nxTop}>
                    <BadgeMedallion badge={badge} earned={false} progressPercent={percent} size="sm" />
                    <span>
                      <span className={styles.nxCat}>{badge.category}</span>
                      <h3>{badge.name}</h3>
                    </span>
                  </span>
                  <span className={styles.nxCrit}>{badge.criteria ?? badge.when}</span>
                  {badge.progress && (
                    <span className={styles.nxProg}>
                      <span className={styles.pips}>
                        {Array.from({ length: Math.min(badge.progress.target, 6) }, (_, pipIndex) => (
                          <span
                            key={pipIndex}
      ...
      {fastestSources.length > 0 && (
        <div className={styles.fast}>
          <span className={styles.fastLabel}>{t("members:badges.momentum.fastestXp")}</span>
      ...
      {previewBadge && (
        <BadgeEarnMoment badge={previewBadge} level={level} onClose={() => setPreviewBadge(null)} />
      )}
    </section>
  );
}
