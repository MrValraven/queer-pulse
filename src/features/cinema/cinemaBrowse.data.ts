...
const BROWSE_POSTER_BY_ID: Record<string, string> = {
  cascais: 'https://images.unsplash.com/photo-1770902895934-b04a10daa893?q=80&w=400&auto=format&fit=crop',
  paris: 'https://images.unsplash.com/photo-1759547020777-14a1ca4c3fdf?q=80&w=400&auto=format&fit=crop',
  pharmacy: 'https://images.unsplash.com/photo-1759773944717-0e3617799860?q=80&w=400&auto=format&fit=crop',
  mother: 'https://images.unsplash.com/photo-1760031009842-9175946953c8?q=80&w=400&auto=format&fit=crop',
  archive: 'https://images.unsplash.com/photo-1761142681497-b2dc51f4e28c?q=80&w=400&auto=format&fit=crop',
  carta: 'https://images.unsplash.com/photo-1761948245703-cbf27a3e7502?q=80&w=400&auto=format&fit=crop',
}
...
function matchAccessibility(film: CinemaFilm, selected: Set<string>): boolean {
  ...
  return [...selected].every((label) =>
    (ACCESS_NEEDLE[label] ?? [label]).some((needle) => haystack.includes(needle.toLowerCase())),
  )
}
...
export function filterFilms(filters: BrowseFilters): CinemaFilm[] {
  return films.filter((f) => {
    ...
    if (filters.country.size > 0 && !matchCountry(f, filters.country)) return false
    if (filters.accessibility.size > 0 && !matchAccessibility(f, filters.accessibility)) return false
