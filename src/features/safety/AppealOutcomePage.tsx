  ...
  return (
    <div className={s.page}>
      <div className={s.stateBar}>
        {STATES.map((st) => (
          <button key={st.id} className={[s.stateBtn, state === st.id && s.stateBtnActive].filter(Boolean).join(' ')} onClick={() => setState(st.id)}>
            {st.label}
          </button>
        ))}
      </div>
        ...
        <div className={`${s.card} ${s.center}`}>
          <div className={s.icon} style={{ background: 'rgba(232,119,90,.1)' }}>
            <svg width={26} height={26} viewBox="0 0 26 26" fill="none">
              ...
              <path d="M13 8v5.5l3 2" stroke="var(--accent)" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
          <div className={s.title}>
            We're <em>reviewing</em> your appeal
          </div>
          <div className={s.sub}>Our moderation team will look at your case carefully and respond within 5 business days.</div>
      ...
      {state === 'overturned' && (
        <div className={`${s.card} ${s.center}`}>
          ...
          <div className={s.title}>
            Your appeal was <em style={{ color: 'var(--jade)' }}>successful</em>
          </div>
          <div className={s.sub}>After reviewing your case, we've reversed the original decision. We're sorry for the inconvenience.</div>
          <div className={s.refBox}>
            <div className={s.csRow}>
              <span className={s.csLabel}>Appeal reference</span>
              <span className={s.refVal}>QP-APP-2847</span>
        ...
        <div className={`${s.card} ${s.center}`}>
          <div className={s.icon} style={{ background: 'rgba(45,27,61,.07)' }}>
            <svg width={26} height={26} viewBox="0 0 26 26" fill="none">
              ...
              <path d="M13 9v5M13 17v.5" stroke="var(--plum)" strokeWidth={2} strokeLinecap="round" />
            </svg>
          </div>
          <div className={s.title}>
            We've <em>reviewed</em> your appeal
          </div>
          <div className={s.sub}>After careful consideration, we've determined that the original decision was appropriate.</div>
          <div className={s.refBox}>
            <div className={s.csRow}>
              <span className={s.csLabel}>Appeal reference</span>
              <span className={s.refVal}>QP-APP-2847</span>
