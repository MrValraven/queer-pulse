...
export function DestructiveActionFlow({
  ...
  return (
    <div
      ...
      onClick={(e) => { if (e.target === e.currentTarget && phase === 'confirm') onClose() }}
    >
      {phase === 'done' ? (
        <div className={styles.result} role="dialog" aria-modal="true" aria-label={content.eyebrow}>
          <div className={styles.resultIcon}><FiCheck /></div>
          <h3 className={styles.resultTitle}>{content.resultTitle}</h3>
          <p className={styles.resultBody}>{content.resultBody}</p>
          <div className={styles.resultBtns}>
            <Button variant="ghost-dark" to={routes.homepage}>Back to QueerPulse</Button>
          </div>
        </div>
      ) : (
        <div className={styles.modal} role="dialog" aria-modal="true" aria-label={content.eyebrow} aria-busy={phase === 'loading'}>
          {phase === 'loading' ? (
            <>
            ...
            <>
              <div className={`${styles.icon} ${content.tone === 'accent' ? styles.iconAccent : styles.iconPlum}`}>
                {content.icon}
              </div>
              ...
              <div className={styles.btns}>
                ...
                <Button variant="primary" onClick={() => setPhase('loading')}>{content.confirmLabel}</Button>
              </div>
            </>
          )}
        </div>
