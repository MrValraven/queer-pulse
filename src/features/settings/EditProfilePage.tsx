...
export function EditProfilePage() {
  ...
  const [savedSections, setSavedSections] = useState<ProfileSection[] | null>(null)
    ...
    <AppShell>
      ...
      {savedSections && (
        <div className={styles.savedBar}>
          <span className={styles.savedIcon}><FiCheck /></span>
          <span className={styles.savedText}>
            Saved.{' '}
            {savedSections.length > 0 ? (
              <>Updated <strong>{savedSections.map((s) => SECTION_LABELS[s] ?? s).join(', ')}</strong>.</>
            ) : (
      ...
      {unsaved && (
        <div className={styles.saveBar}>
          <span className={styles.unsavedLabel}>
            Unsaved changes in {[...changed].map((s) => SECTION_LABELS[s] ?? s).join(', ')}
          </span>
