...
/** The mock handed to `moderation.setReportTarget`, typed to mirror the real
 *  `Dispatch<SetStateAction<ForumReportTarget | null>>` from
 *  `useThreadModeration` so the calls made against it are typed too. */
type SetReportTargetMock = ReturnType<
  typeof vi.fn<(target: ForumReportTarget | null) => void>
>;
...
function moderationStub(setReportTarget: SetReportTargetMock) {
  return {
    setReportTarget,
...
function renderOp(thread: Thread, setReportTarget: SetReportTargetMock) {
...
describe("ThreadOpSection report target", () => {
  it("reports the OP's real opPostId, not the synthetic thread id", async () => {
    const setReportTarget = vi.fn<(target: ForumReportTarget | null) => void>();
    renderOp(
      makeThread({ id: 42, opPostId: "op-real-post-abc" }),
      setReportTarget,
    );
