...
type PersonaRotationOptions<Key> = {
...
/**
 * Advances the personas showcase on a timer until the reader takes over.
 *
 * The timer only runs while the section is actually on screen and nobody is
 * hovering or keyboard-focused inside it, so a persona never swaps out from
 * under someone mid-read. Any persona interaction sets `isStopped` upstream
 * and rotation never resumes.
 *
 * Returns the ref to put on the section element plus the pause handlers to
 * spread onto it.
 */
export function usePersonaRotation<Key>({
  order,
  selectedKey,
  onRotate,
  isStopped,
  isEnabled,
}: PersonaRotationOptions<Key>) {
  ...
  useEffect(() => {
    ...
    }, PERSONA_ROTATION_INTERVAL_MS);
    return () => {
      window.clearInterval(timer);
    };
  }, [isRunning, order, selectedKey]);
