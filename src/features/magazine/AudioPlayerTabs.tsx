...
export function NotesTab() {
  return (
    <div className={styles.notesText}>
      ...
      <p>
        This conversation is about <strong>the fifteen minutes</strong> that
        most trans patients spend, every visit, explaining themselves before any
      ...
      <p>
        ...
        Farmácia do Carmo · the{" "}
        <Link to={ARTICLE}>cover piece of Issue 09</Link> by Sara Pinheiro,
        which prompted the recording.
      </p>
      <p>
      ...
      {CHAPTERS.map((c, i) => {
        ...
        return (
          <button
            type="button"
            key={c.time}
            className={[styles.chapter, current && styles.chapterCurrent]
              .filter(Boolean)
              .join(" ")}
            onClick={() => player.seek(c.sec)}
          >
  ...
  return (
    <>
      <div className={styles.trControls}>
        ...
        <button
          type="button"
          className={[styles.actionBtn, autoScroll && styles.actionActive]
            .filter(Boolean)
            .join(" ")}
      ...
      <div className={styles.transcript}>
        {TRANSCRIPT.map((t, i) => (
          <div
            ...
            className={[styles.trBlock, currentIndex === i && styles.trCurrent]
              .filter(Boolean)
              .join(" ")}
      ...
      {downloadOpen && (
        <TranscriptDownloadModal onClose={() => setDownloadOpen(false)} />
      )}
    </>
  );
}
