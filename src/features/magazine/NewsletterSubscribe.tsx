...
export function NewsletterSubscribe({ stream }: Props) {
  ...
  const [done, setDone] = useState<{ email: string; stream: Stream | 'all' } | null>(null)
      ...
      <div className={styles.subDone} role="status">
        ...
        <div>
          ...
          <p className={styles.subDoneNext}>
            <FiMail aria-hidden /> Check your inbox to confirm — the link expires in 48
            hours. You can change which streams you get from any email.
          </p>
    ...
    <>
      ...
      <p className={styles.subFoot}>
        {stream === 'all' ? (
          ...
          <>
            You'll get <strong>{STREAM_LABEL[stream]}</strong> — adjust in step 2.{' '}
          </>
        )}
