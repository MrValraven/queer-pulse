...
export function useAudioPlayer(): AudioPlayer {
  ...
  const clamp = useCallback((sec: number) => Math.max(0, Math.min(DURATION_SEC, sec)), [])

  const seek = useCallback((sec: number) => setCurrentTime(clamp(sec)), [clamp])
  const seekFraction = useCallback(
    (fraction: number) => setCurrentTime(clamp(fraction * DURATION_SEC)),
    [clamp],
  )
  const nudge = useCallback((delta: number) => setCurrentTime((t) => clamp(t + delta)), [clamp])
