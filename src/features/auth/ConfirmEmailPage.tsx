  ...
  return (
    <AuthLayout>
      ...
      <p className={styles.sub} style={{ marginBottom: 0 }}>
        We sent a six-digit code to <strong style={{ color: 'var(--ink)', fontWeight: 600 }}>{DEMO_EMAIL}</strong>. It expires in <strong style={{ color: 'var(--ink)', fontWeight: 600 }}>10 minutes</strong>.
      </p>

      <div className={[pageStyles.codeRow, error ? pageStyles.codeRowError : ''].filter(Boolean).join(' ')} onPaste={handlePaste}>
        {digits.map((d, i) => (
          <input
            key={i}
      ...
      <div className={[pageStyles.codeMsg, messageOk ? pageStyles.codeMsgOk : ''].filter(Boolean).join(' ')}>
        {message}
      </div>
