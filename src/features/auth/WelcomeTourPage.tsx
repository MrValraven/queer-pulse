        ...
        <div className={styles.card}>
          <div className={styles.progress}>
            <div className={styles.bar}>
              <div className={styles.fill} style={{ width: TOUR_STEP_FILLS[step - 1] }} />
            </div>
            <div className={styles.stepsLabel}>{TOUR_STEP_LABELS[step - 1]}</div>
          </div>

          <div key={step} className={styles.step}>
            {step === 1 && <TourWelcome onNext={() => go(2)} />}
            {step === 2 && <TourProfile onNext={() => go(3)} onBack={() => go(1)} />}
            {step === 3 && <TourInterests onNext={() => go(4)} onBack={() => go(2)} />}
            {step === 4 && <TourCommunities onNext={() => go(5)} onBack={() => go(3)} />}
            {step === 5 && <TourConnections onNext={() => go(6)} onBack={() => go(4)} />}
            {step === 6 && <TourExplore />}
          </div>
        </div>
      </div>
