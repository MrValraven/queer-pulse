  ...
  return (
    <>
      ...
      <p className={styles.p}>
        Somebody in the community thought you belonged here — and that's how everyone arrived.
...
export function TourProfile({ onNext, onBack }: StepProps) {
  ...
  return (
    <>
      ...
      <p className={styles.p}>This is how the community will know you. You can change everything later.</p>
      <div className={styles.fields}>
        <div className={styles.row}>
          <input className={styles.input} type="text" placeholder="Your name" />
          <input className={styles.input} type="text" placeholder="Pronouns (optional)" />
        </div>
        <input className={styles.input} type="text" placeholder="What you do — your role or practice" />
      ...
      <div className={styles.intGrid}>
        {INTERESTS.map((interest) => (
          <button
            key={interest.label}
            type="button"
            className={[styles.intItem, selected.has(interest.label) && styles.intSel]
              .filter(Boolean)
              .join(' ')}
    ...
    <>
      ...
      <div className={styles.commList}>
        {TOUR_COMMUNITIES.map((community) => (
          <label
            key={community.name}
            className={[styles.commPick, joined.has(community.name) && styles.commPicked]
              .filter(Boolean)
              .join(' ')}
          >
            <input
        ...
        {CONNECTIONS.map((person) => {
          ...
          return (
            <div key={person.name} className={styles.sugCard}>
              <div className={[styles.sugAv, styles[person.tint]].join(' ')}>{person.initials}</div>
