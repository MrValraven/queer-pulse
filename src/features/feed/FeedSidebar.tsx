...
import { Avatar, AvatarStack, SkeletonAvatar, SkeletonLine } from '../../shared/components/ui'
...
function UpcomingRowSkeleton() {
  return (
    <div className={styles.upcomingRow} aria-hidden>
      <SkeletonLine width={46} height={22} style={{ borderRadius: 6, flex: 'none' }} />
      <div style={{ flex: 1 }}>
...
export function FeedSidebar({ loading = false }: { loading?: boolean }) {
  ...
  return (
    <aside className={styles.sidebar}>
      <div className={styles.sbCard}>
        ...
        {loading ? (
          Array.from({ length: 2 }).map((_, i) => <UpcomingRowSkeleton key={i} />)
        ) : (
          <>
            <Link to={gatheringPath('queer-night-swim')} className={`${styles.upcomingRow} ${styles.revealRow}`}>
            ...
            <Link to={gatheringPath('queer-book-club')} className={`${styles.upcomingRow} ${styles.revealRow}`} style={{ animationDelay: '60ms' }}>
      ...
      <div className={styles.sbCard}>
        ...
        {loading
          ...
          : NEW_THIS_WEEK.map((person, i) => (
              <div key={person.name} className={`${styles.sbMemberRow} ${styles.revealRow}`} style={{ animationDelay: `${i * 50}ms` }}>
