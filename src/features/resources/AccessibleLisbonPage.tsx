  ...
  return (
    <PageShell>
      <ResourceHero
        ...
        title={<>Lisbon, <em>actually reachable.</em></>}
      ...
      {GROUPS.map((group, gi) => (
        ...
        >
          <div className="wrap">
            ...
            <Reveal as="p" className={styles.leadP}>{group.intro}</Reveal>
            <div className={styles.grid}>
              {group.places.map((place, i) => (
                <Reveal key={place.name} className={styles.card} delay={i * 55}>
                  ...
                  <div className={styles.cardName} style={{ fontSize: 19 }}>{place.name}</div>
      ...
      <Outro
        title={<>Been somewhere <em>that works?</em></>}
