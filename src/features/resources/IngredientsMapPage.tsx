  ...
  return (
    <PageShell>
      ...
      <section className={`${styles.section} ${styles.sectionPaper}`}>
        <div className="wrap">
          {INTRO.map((p) => (
            <Reveal as="p" key={p} className={styles.leadP} style={{ maxWidth: '64ch', marginBottom: 48 }}>{p}</Reveal>
          ))}
              ...
              <div className={styles.grid}>
                {area.spots.map((s, i) => (
                  <Reveal key={s.name} className={styles.card} delay={i * 45}>
                    <div className={styles.cardName} style={{ fontSize: 18 }}>{s.name}</div>
          ...
          <div className={`${styles.card} ${styles.cardDashed}`} style={{ marginTop: 32 }}>
            Know a spot we're missing?
            <Button
              variant="ghost"
              style={{ marginTop: 12 }}
              onClick={() => showToast('Added to the queue — a mod will pop it on the map. Obrigada!')}
            >
              Add your spot
            </Button>
          </div>
      ...
      <Outro
        title={<>Joy is as <em>political</em> as solidarity.</>}
