  ...
  return (
    <PageShell>
      <ResourceHero
        ...
        title={<>What we mean by <em>spoons.</em></>}
      ...
      <section className={`${styles.section} ${styles.sectionPaper}`} id="what">
        <div className="wrap">
          <Reveal as="h2">What it <em>is</em></Reveal>
          <Reveal>
            <div className={styles.spoonRow} aria-hidden>
              {SPOONS.map((spent, i) => (
                <GiSpoon key={i} className={`${styles.spoon} ${spent ? styles.spoonSpent : ''}`} />
              ))}
            </div>
      ...
      <section className={`${styles.section} ${styles.sectionCream}`} id="uses">
        <div className="wrap">
          <Reveal as="h2">How we <em>use it</em></Reveal>
          <div className={styles.grid}>
            {USES.map((u, i) => (
              <Reveal key={u.title} className={styles.card} delay={i * 55}>
                <div className={styles.cardName} style={{ fontSize: 18 }}>{u.title}</div>
      ...
      <section className={`${styles.section} ${styles.sectionPaper}`} id="rsvp">
        <div className="wrap">
          <Reveal as="h2">When you <em>RSVP</em></Reveal>
          <div className={styles.checklist}>
            {RSVP_TIPS.map((t) => (
              <Reveal key={t} className={styles.checkItem} style={{ gridTemplateColumns: '1fr' }}>
                <div className={styles.cardSpec} style={{ flex: 'none' }}>{t}</div>
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <Outro
        title={<>Come in whatever <em>state you're in.</em></>}
