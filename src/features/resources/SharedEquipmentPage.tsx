  ...
  return (
    <PageShell>
      <ResourceHero
        ...
        title={<>Shared kit, <em>shared care.</em></>}
      ...
      <section className={`${styles.section} ${styles.sectionPaper}`} id="kit">
        <div className="wrap">
          <Reveal as="h2">The <em>kit</em></Reveal>
          ...
          <div className={styles.grid}>
            {EQUIPMENT.map((item, i) => (
              <Reveal key={item.name} className={styles.card} delay={i * 55}>
                <ImageSlot tint={item.tint} placeholder={item.name} height={160} />
                <div className={styles.cardName} style={{ fontSize: 19, marginTop: 4 }}>{item.name}</div>
                ...
                <div className={styles.cardFoot}>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      showToast(`Request sent for the ${item.name} — a mod will confirm your slot.`)
                    }
                  >
      ...
      <section className={`${styles.section} ${styles.sectionCream}`} id="care">
        <div className="wrap">
          <Reveal as="h2">How we <em>care for it</em></Reveal>
          <div className={styles.checklist}>
            {CARE.map((c) => (
              <Reveal key={c} className={styles.checkItem} style={{ gridTemplateColumns: '1fr' }}>
                <div className={styles.cardSpec} style={{ flex: 'none' }}>{c}</div>
              </Reveal>
            ))}
          </div>
        </div>
