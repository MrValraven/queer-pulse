  ...
  return (
    <>
      <div className={sx('modal-head')}>
        ...
        <h2 className={sx('modal-title')}>Block <em>{block.host || 'this host'}?</em></h2>
      </div>
      <div className={sx('modal-body')}>
        <p className={sx('modal-evname')}>
          You won’t see events from {who} again, and they won’t be able to invite you.
          You can undo this any time in your settings.
        </p>
      </div>
      <div className={sx('modal-foot')}>
        <Button variant="ghost" onClick={closeBlock}>Cancel</Button>
        <Button variant="danger" onClick={confirmBlock}>Block</Button>
      </div>
    </>
  )
}
