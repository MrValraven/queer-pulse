  ...
  return (
    <div className={sx('notif-panel')} role="dialog" aria-label="Notifications">
      <div className={sx('notif-head')}>
        ...
        <button type="button" className={sx('notif-clear')} onClick={markAllRead}>Mark all read</button>
      </div>
      <div className={sx('notif-list')}>
        {notifs.length === 0 ? (
          ...
          notifs.map((n, i) => (
            <button key={n.id} type="button" className={sx(`notif-item${n.unread ? ' unread' : ''}`)} onClick={() => notifGo(i)}>
              <span className={sx('ni-dot')} />
              <span className={sx('ni-body')}>
                <span className={sx('ni-text')}>{n.lead}<strong>{n.bold}</strong>{n.tail}</span>
