...
export function DayFilterChip() {
  ...
  return (
    <div className={`${sx('dayfilter')} ${sx('show')}`}>
      <span className={sx('dayfilter-label')}>Showing <strong>{label}</strong></span>
      <button type="button" className={sx('dayfilter-clear')} onClick={clearDay}>
        <svg width="11" height="11" viewBox="0 0 11 11" fill="none" aria-hidden><path d="M2 2l7 7M9 2l-7 7" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" /></svg>
        Show all
      </button>
    </div>
  )
