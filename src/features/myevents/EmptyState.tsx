  ...
  return (
    <div className={sx('empty')}>
      <div className={sx(`empty-icon ${cfg.tone || ''}`)}>
        <svg viewBox="0 0 28 28" aria-hidden>{cfg.icon}</svg>
      </div>
      ...
      <div className={sx('empty-actions')}>
        {cfg.actions.map((a) => (
          a.to
            ? <Button key={a.label} variant={a.variant} to={a.to}>{a.label}</Button>
            : <Button key={a.label} variant={a.variant} onClick={() => (a.reset === 'day' ? clearDay() : clearSecondary())}>{a.label}</Button>
        ))}
      </div>
    </div>
  )
