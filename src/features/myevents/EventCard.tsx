...
import {
  AccessRow, AlertStrip, ConflictNote, EdgeNote, SeriesLine, DayOfPanel,
...
export function EventCard({ ev }: { ev: MyEvent }) {
  ...
  const soon = !!(isToday(ev) && COMMITTED[ev.cat] && !ev.cancelled && soonLabel(ev))
  ...
  useEffect(() => {
    if (focused) cardRef.current?.scrollIntoView({ behavior: reduce ? 'auto' : 'smooth', block: 'center' })
  }, [focused, reduce])

  const cardCls = sx(`ev-card ${ev.cat}${ev.cancelled ? ' cancelled' : ''}${soon ? ' soon' : ''}${removingId === ev.id ? ' removing' : ''}${focused ? ' flash' : ''}`)
  ...
  return (
    <div ref={cardRef} className={cardCls} data-id={ev.id}>
      <div
        ...
        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleSelect(ev.id) } }}
      >
        {Icons.check}
      </div>
      ...
      <div className={sx('ev-body')}>
        ...
        <EventFoot ev={ev} />
        <EventTools ev={ev} dayofShown={dayofShown} onToggleDayof={() => setDayofShown((s) => !s)} />
      </div>
