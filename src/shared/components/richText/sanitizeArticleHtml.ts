/**
 * Sanitizes block-authored rich text before it is ever rendered, guarding the
 * read path for both magazine article blocks (`ArticleBlockView.tsx`) and
 * resource guide blocks (`GuideBody`), each via `dangerouslySetInnerHTML`.
 * The backend also sanitizes on write with the same allowlist, in
 * `queerpulse-backend/src/common/inline-html.ts`.
 *
