...
/** Event a keyboard shortcut dispatches to open the toolbar's link field for
 * the current selection (Cmd/Ctrl+K in the guide workspace). */
export const RICH_TEXT_LINK_REQUEST_EVENT = "qp-rich-text-link-request";

/** Asks any mounted `SelectionToolbar` whose scope holds the selection to
 * open its link field. A no-op when nothing is selected. */
export function requestLinkPrompt(): void {
  document.dispatchEvent(new Event(RICH_TEXT_LINK_REQUEST_EVENT));
}
