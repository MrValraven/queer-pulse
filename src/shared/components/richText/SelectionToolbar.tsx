...
import { useRichTextLinkRequest } from "./useRichTextLinkRequest";
...
export function SelectionToolbar({ scopeRef }: SelectionToolbarProps) {
  ...
  // Cmd/Ctrl+K from a caller opens the address field for the live selection.
  // It positions the toolbar too, since the toolbar may be hidden when the
  // request arrives and a hidden toolbar would never show the field.
  const openLinkPromptForRange = useCallback((range: Range) => {
    const rect = range.getBoundingClientRect();
    if (rect.width === 0 && rect.height === 0) return;
    setPosition({ x: rect.left + rect.width / 2, y: rect.top });
    setLinkRange(range);
  }, []);
  useRichTextLinkRequest(scopeRef, openLinkPromptForRange);
  ...
  return (
    <div
      ...
      aria-label={t("shared:richText.toolbarAria")}
