...
import {
  parseDate, formatIsoDate, addDays, addMonths, getDaysInMonth,
  weekdayOf, compareDate, isSameDate, firstDayOfWeek, startOfWeek, todayPlain,
} from "./plainDate";
...
describe("plainDate", () => {
  ...
  it("compareDate / isSameDate", () => {
    expect(compareDate({ year: 2026, month: 1, day: 1 }, { year: 2026, month: 1, day: 2 })).toBeLessThan(0);
    expect(isSameDate({ year: 2026, month: 1, day: 1 }, { year: 2026, month: 1, day: 1 })).toBe(true);
  ...
  it("startOfWeek respects weekStart", () => {
    ...
    expect(startOfWeek({ year: 2026, month: 3, day: 5 }, 0)).toEqual({ year: 2026, month: 3, day: 1 });
    // week starting Monday -> 2026-03-02
    expect(startOfWeek({ year: 2026, month: 3, day: 5 }, 1)).toEqual({ year: 2026, month: 3, day: 2 });
  });
