...
export function ProfileThemeProvider({ children }: { children: ReactNode }) {
  ...
  return <ProfileThemeContext.Provider value={value}>{children}</ProfileThemeContext.Provider>
}

export function useProfileTheme(): ProfileThemeContextValue {
  const ctx = useContext(ProfileThemeContext)
  if (!ctx) throw new Error('useProfileTheme must be used within a ProfileThemeProvider')
  return ctx
}
