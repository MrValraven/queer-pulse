...
export function WorkProfileProvider({ children }: { children: ReactNode }) {
  ...
  const toggleTransSupport = useCallback((id: string) => {
    setTransSupport((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]))
  }, [])

  const value = useMemo(
    () => ({ outAtWork, setOutAtWork, transSupport, toggleTransSupport, safeOnly, setSafeOnly }),
    [outAtWork, transSupport, toggleTransSupport, safeOnly],
  )

  return <WorkProfileContext.Provider value={value}>{children}</WorkProfileContext.Provider>
}
