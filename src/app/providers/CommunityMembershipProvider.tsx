...
import type { CommunityRole, Membership } from '../../features/communities/membership.types'
...
export function CommunityMembershipProvider({ children }: { children: ReactNode }) {
  const [memberships, setMemberships] = useState<Record<string, Membership>>(SEED)
  const [pendingRequests, setPendingRequests] = useState<string[]>([])
  const [promotedMods, setPromotedMods] = useState<Record<string, string[]>>({})

  const isMember = useCallback((slug: string) => slug in memberships, [memberships])
  const hasRequested = useCallback((slug: string) => pendingRequests.includes(slug), [pendingRequests])
  ...
  const join = useCallback((slug: string) => {
    setMemberships((prev) =>
      prev[slug] ? prev : { ...prev, [slug]: { role: 'member', joinedAt: 'just now' } },
    )
    setPendingRequests((prev) => prev.filter((s) => s !== slug))
  }, [])

  const requestToJoin = useCallback((slug: string) => {
    setPendingRequests((prev) => (prev.includes(slug) ? prev : [...prev, slug]))
  }, [])
...
export function useCommunityMembership() {
  ...
  if (!ctx) {
    throw new Error('useCommunityMembership must be used within CommunityMembershipProvider')
  }
  return ctx
}
