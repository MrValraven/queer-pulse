...
export default defineConfig([
  ...
  // Same argument, one file over: the composer's emoji picker is a grid OF
  // emoji, so its dataset (generated by scripts/generate-emoji-data.mjs from
  // emojibase-data) is the feature's data, not chrome, and react-icons has
  // nothing to offer it. `**/*.data.{ts,tsx}` above already exempts it from
  // `local/no-literal-string`.
  //
  // `local/no-glyph-icon` comes off here too, and ONLY here. 27 of the 1,914
  // entries sit in the GLYPH_ICON blocks because Unicode put them there:
  // ⌚ ⌛ ⏰ ⏳ ⏱ ⏲ ⌨ in Miscellaneous Technical, the media transport set
  // (▶ ◀ ⏩ ⏪ ⏭ ⏮ ⏯ ⏸ ⏹ ⏺ ⏏ ⏫ ⏬) split across it and Geometric Shapes, the six
  // square glyphs, and Ⓜ in Enclosed Alphanumerics. Every one is a real emoji
  // carrying U+FE0F, picked from a grid and sent in a message. The rule is
  // right about a ⏱ standing in for a duration icon on a meta pill; it has
  // nothing to say about the emoji a person chose to type.
  {
    files: ["src/features/messages/emoji.data.ts"],
    rules: { "local/no-emoji": "off", "local/no-glyph-icon": "off" },
  },
