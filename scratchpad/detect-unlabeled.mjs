...
for (const file of files) {
    ...
    if (
      ...
      if (ts.isStringLiteral(init)) htmlForTargets.add(init.text);
        ...
        if (ts.isStringLiteral(init.expression))
          htmlForTargets.add(init.expression.text);
    ...
    for (const prop of opening.attributes.properties) {
      ...
      if (ts.isJsxAttribute(prop)) {
        ...
        if (init && ts.isStringLiteral(init)) val = init.text;
          ...
          val = ts.isStringLiteral(init.expression)
  ...
  function walk(node) {
    ...
    if (ts.isJsxElement(node)) {
      ...
      el = opening.tagName.getText();
    } else if (ts.isJsxSelfClosingElement(node)) {
      opening = node;
      el = node.tagName.getText();
    }
    ...
    if (opening && (el === "input" || el === "select" || el === "textarea")) {
      ...
      if (!(
        ...
        if (!hasAria && !idLabelled && !wrapped && !hasSpread) {
          ...
          hits.push(
            `${file}:${line}  <${el}${id ? ` id=${JSON.stringify(id)}` : ""}${type ? ` type=${type}` : ""}>`,
          );
        }
      }
    }
