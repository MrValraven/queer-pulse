...
- **The scroll is sacred.** Key rows by stable message ID (never index). Loading older history must not move the viewport (end-anchor + height compensation in `useLayoutEffect`). Auto-scroll to bottom only when the user is near the bottom; otherwise a jump-to-latest pill with a count. Stick-to-bottom survives bubble _resizes_, not just new messages.
