...
skill's job. Reach for `motion` when the interaction is *interactive or physical*,
not decorative.
...
OS setting AND the toggle and re-renders on change. Gate the *decoration*, keep the
*function*: a 1:1 finger-driven drag stays (it's a control); only its glide/lift
