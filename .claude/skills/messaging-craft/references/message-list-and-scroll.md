...
When you prepend older messages, the browser's default is to keep `scrollTop` constant, which means all the new content pushes the visible messages *down* — the thread jumps. The fix is to anchor to a stable item:
...
- Tight vertical spacing *inside* a run; larger gap *between* runs and a clear gap when the author changes.
